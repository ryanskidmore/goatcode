...
import {
  resolveLegacyProjectConfigPath,
...
export async function loadConfig(projectDir: string): Promise<GoatCodeConfig | null> {
  ...
  const userConfig =
    userConfigPath && existsSync(userConfigPath)
      ? await loadValidatedConfigFile(userConfigPath)
      : null;
  ...
  if (!hasProjectConfig) {
    ...
    if (existsSync(legacyConfigPath)) {
      log(
        "[config/loader] Found legacy ochead.config.ts — please rename it to goatcode.config.ts",
        {
