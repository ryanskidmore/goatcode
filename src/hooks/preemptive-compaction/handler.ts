...
type PreemptiveCompactionDeps = {
  threshold?: number
  getTokenUsage?: (input: PreemptiveCompactionInput) => { usedTokens: number; contextLimit: number } | undefined
  compactSession?: (sessionID: string, info: { usageRatio: number; usedTokens: number; contextLimit: number }) => void | Promise<void>
}

function resolveTokenUsage(input: PreemptiveCompactionInput): { usedTokens: number; contextLimit: number } | undefined {
