  ...
  describe("#given a valid tmux subcommand", () => {
    describe("#when the command succeeds", () => {
      it("#then it returns stdout output", async () => {
        spawnSpy.mockReturnValue(makeProc("session created\n", "", 0) as ReturnType<typeof Bun.spawn>)

        const result = await interactiveBashTool.execute({ tmux_command: "new-session -d -s myapp" }, mockContext)
    ...
    describe("#when the command produces no output", () => {
      it("#then it returns the no-output placeholder", async () => {
        ...
        const result = await interactiveBashTool.execute({ tmux_command: "send-keys -t myapp q Enter" }, mockContext)
    ...
    describe("#when the command fails with stderr", () => {
      it("#then it returns the error message from stderr", async () => {
        spawnSpy.mockReturnValue(makeProc("", "no server running on /tmp/tmux-1000/default", 1) as ReturnType<typeof Bun.spawn>)

        const result = await interactiveBashTool.execute({ tmux_command: "send-keys -t missing q" }, mockContext)
    ...
    describe("#when the command fails with no stderr", () => {
      it("#then it returns a generic exit code error", async () => {
        ...
        const result = await interactiveBashTool.execute({ tmux_command: "kill-session -t myapp" }, mockContext)
  ...
  describe("#given a quoted tmux subcommand", () => {
    describe("#when the command contains quoted arguments", () => {
      it("#then it tokenizes quotes correctly and passes args to tmux", async () => {
        ...
        await interactiveBashTool.execute({ tmux_command: 'send-keys -t myapp "vim file.ts" Enter' }, mockContext)
  ...
  describe("#given tmux is not installed", () => {
    describe("#when spawn throws ENOENT", () => {
      it("#then it returns a helpful not-installed error", async () => {
        ...
        const result = await interactiveBashTool.execute({ tmux_command: "new-session -d -s test" }, mockContext)

        expect(result).toBe("Error: tmux is not installed or not found in PATH. Install tmux to use this tool.")
      })
    })
  })
})
