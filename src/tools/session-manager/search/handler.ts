...
export async function handleSessionSearch(args: SessionSearchArgs, ctx: OpenCodeContext): Promise<string> {
  try {
    ...
    const searchOperation = async (): Promise<SearchMatch[]> => {
      ...
      for (const session of sessionsToScan) {
        ...
        const results = await searchInSession(ctx, session.id, args.query, caseSensitive, remaining)
