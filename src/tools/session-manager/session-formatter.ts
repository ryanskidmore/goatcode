...
import type { SessionSummary, MessageSummary, SearchMatch, SessionDetail } from "./types"
...
export function extractAgentsFromMessages(messages: Array<{ info: Message; parts: Part[] }>): string[] {
  ...
  for (const { info, parts } of messages) {
    ...
    for (const part of parts) {
      if (part.type === "text") {
        ...
        const tp = part as { type: "tool"; tool: string; state: { status: string; input?: Record<string, unknown> } }
...
export function formatSearchResults(matches: SearchMatch[]): string {
  ...
  const lines: string[] = [`Found ${matches.length} match${matches.length !== 1 ? "es" : ""} across sessions:\n`]
...
export function formatSessionDetail(detail: SessionDetail): string {
  ...
  if (detail.hasTodos) {
    lines.push(`Has Todos: Yes (${detail.todoCount} items, ${detail.completedTodoCount} completed)`)
  } else {
...
export function buildSessionDetail(
  ...
  const durationLabel = firstMs !== null && lastMs !== null ? computeDurationLabel(firstMs, lastMs) : null
  const agentsUsed = extractAgentsFromMessages(messages)
...
export function searchMessages(
  ...
  for (const { info, parts } of messages) {
    ...
    for (const part of parts) {
      ...
      const rawText = (part as { type: "text"; text: string }).text
