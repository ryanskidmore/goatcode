...
const categoryListForDescription = CATEGORY_NAMES
  .map((name) => `"${name}"`)
  .join(", ")

const taskArgsSchema = z.object({
  category: z.string().describe(
    `Category to route to. One of: ${categoryListForDescription}`,
  ),
  ...
  run_in_background: z.boolean().describe(
    "true: async (returns task_id), false: sync (waits for result)",
  ),
