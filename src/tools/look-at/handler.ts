...
export function createLookAtTool(poller: Poller = pollUntilStable): ToolDefinition {
  return tool({
    ...
    execute: async (args, toolContext) => {
      ...
      if (filePath && imageData) return "Error: Provide only one of 'file_path' or 'image_data', not both."
      ...
      if (imageData) {
        ...
        filePart = { type: "file", mime, url: `data:${mime};base64,${base64}`, filename: `clipboard-image.${mime.split("/")[1] ?? "png"}` }
        ...
        if (isBinaryPath(filePath)) {
          ...
          filePart = { type: "file", mime, url: `data:${mime};base64,${base64}`, filename: filePath.split("/").pop() ?? "file" }
      ...
      try {
        await poller(async () => {
          ...
          const statusData = statusResult.data as Record<string, { type?: string }> | undefined
          return { messageCount: messages.length, isIdle: statusData?.[sessionId]?.type === "idle" }
        })
      } catch (pollError) {
