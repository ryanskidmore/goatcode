...
| T60 | rename with empty newName | `newName: ""` | Execute | Zod validation rejects — `.min(1)` enforced *(fixed)* |
| T61 | LSP client unavailable | No LSP server | Any LSP tool | Returns `"LSP client method unavailable"` |
| T62 | Float line numbers | `line: 1.5` | Execute | Zod validation rejects — `.int()` enforced on all LSP position args *(fixed)* |
| T63 | Dispatch strategy fallback | Primary method unavailable | Execute | Falls through 5-strategy chain |
| T64 | Prepare_rename returns [] | Empty array result | Execute | Returns `"Rename is not valid at this position"` — empty array handled same as null *(fixed)* |
| T65 | `unwrapClientResponse` with `error: 0` | Response has falsy error | Execute | Does not throw — truthy check `if (record.error)` used *(fixed)* |
...
| T106 | session_list invalid date | `from_date: "not-a-date"` | Execute | Returns all sessions — `Number.isNaN` guard keeps sessions when date is unparseable *(fixed)* |
...
| T112 | session_read include_transcript | `include_transcript: true` | Execute | Parameter removed from schema entirely *(fixed)* |
...
| T121 | session_info hasTranscript | Any session | Execute | `hasTranscript` field removed from output *(fixed)* |
...
| T143 | Loader throws | Loader function throws Error | Execute | Returns `"Error loading skill '\{name\}': \{message\}"` — loader wrapped in try/catch *(fixed)* |
...
| T151 | Large file | Multi-MB text file | Execute | Returns error for files >1 MB — `file.size` checked before `file.text()` *(fixed)* |
...
| F12 | Concurrency: 10 tasks on same model per depth | 10 tasks launched for same model+depth | 11th task | Queues behind (blocks in acquire); depth-0 and depth-1 pools are independent |
...
| F15 | Concurrency starvation | 4 parents hold slots, 17 children queue | Children | Depth-keyed pools prevent starvation — parents (`model:0`) and children (`model:1`) have separate semaphores *(fixed in PR #46)* |
